      ...
      <ul className="items-center justify-center md:flex md:space-x-6 md:space-y-0">
        {HEADER_PATHS.map((path) => (
          <li className="text-gray-600" key={path.title}>
            <Link className="flex h-12 min-h-full w-full items-center rounded-md pl-2 font-semibold hover:bg-gray-200 hover:text-asset-blue dark:text-white dark:hover:bg-gray-700 md:px-2" href={path.href}>
              {path.title}
            </Link>
          </li>
        ))}
